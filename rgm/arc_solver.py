...
#     ok = True
#     if top > 0:
#         ok &= np.all(out[:top, :] == color)
#     if bottom > 0:
#         ok &= np.all(out[Ho - bottom :, :] == color)
#     if left > 0:
#         ok &= np.all(out[:, :left] == color)
#     if right > 0:
#         ok &= np.all(out[:, Wo - right :] == color)
#     if ok:
#         inner = out[
#             top : Ho - bottom if bottom > 0 else Ho,
#             left : Wo - right if right > 0 else Wo,
#         ]
...
# def transform_mask(
#     mask: np.ndarray, rotation_k: int = 0, flip_h=False, flip_v=False
# ) -> np.ndarray:
#     m = mask.copy()
#     if flip_h:
#         m = np.fliplr(m)
...
# def place_mask(
#     canvas: Grid, mask: np.ndarray, color: int, top_left: Tuple[int, int]
# ) -> None:
#     H, W = canvas.shape
#     mh, mw = mask.shape
#     r0, c0 = top_left
...
#             canvas[rr0:rr1, cc0:cc1] = np.where(
#                 mask[mr0:mr1, mc0:mc1], color, canvas[rr0:rr1, cc0:cc1]
#             )
#     else:
...
#         return (
#             GlobalRule(recolor_map={}, add_border=border, output_resize=output_resize),
#             [],
#         )
...
#                     flip_h=fh,
#                     flip_v=fv,
#                     recolor_to=recolor_map.get(oi.color),
#                 ),
#             )
#         )
...
#         m = transform_mask(
#             m, rotation_k=rule.rotation_k, flip_h=rule.flip_h, flip_v=rule.flip_v
#         )
...
#         train_pairs = [
#             (grid_from_list(p["input"]), grid_from_list(p["output"]))
#             for p in task["train"]
#         ]
...
#         preds = [
#             apply_rule_to_grid(ti, global_rule, background=self.background)
...
from arc_rules import infer_rule
