...
def main():
    ...
    ap.add_argument("--single_group", action="store_true")
    ap.add_argument("--trans_r", type=int, default=1)
    ap.add_argument("--no_force_hierarchy", action="store_true")
    ...
    for i, p in enumerate(paths, 1):
        ...
        try:
            ...
            preds1 = solver.solve_task(task)
            preds2 = (
                preds1  # keep identical unless you wire a rule baseline as attempt_2
            )
            ...
            forced = solver._rg_forced_levels if solver._rg_forced_levels else [1]
            print(
                f"[{i}/{len(paths)}] ✓ {tid}{msg} (controls_used={solver._controls_used}, actions={solver.actions.count}, k={solver.kmeans.n_clusters if solver.kmeans else None}, RG_levels={forced})"
            )
