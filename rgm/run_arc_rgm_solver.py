...
def _learn_motion_and_colors(
    ...
    for xi, yi in train_pairs:
        ...
        for i, ox in enumerate(objs_x):
            j = match_map.get(i, None)
            if j is None:
                # fallback: no motion/color info for this object
                continue
            oy = objs_y[j]
...
class ARCRGMSolver:
    ...
    def fit_task(self, task: Dict[str, Any]) -> None:
        ...
        # 2) Build one-hot sequences per OBJECT (NOT per pair):
        #    For each input object in each train pair, create a modality with T=2:
        #    t=0 token from the input object, t=1 token from its matched output object (if any; else fallback).
        seq_tokens: List[np.ndarray] = []  # each is shape (T=2, k)
        locs_xy: List[Tuple[float, float]] = (
            []
        )  # one per modality (from the input object's centroid)
        ...
        for xi, yi in train_pairs:
            ...
            # match x->y within this pair (TDA Wasserstein)
            matches, _ = wasserstein_matching(objs_x, objs_y, return_matrix=False)
            match_map = {i: j for (i, j) in matches}
            ...
            for i, ox in enumerate(objs_x):
                ...
                if j is not None:
                    ...
                    # fallback: identity token if unmatched
                    onehots[1, tok_x] += 1
        ...
        if len(seq_tokens) == 0:
            ...
            # stack modalities: each input object instance is a modality
            # shape: (M, 2, k)
            one_hots = np.stack(seq_tokens, axis=0)
        ...
        agents, RG, _ = spm_mb_structure_learning(
            ...
            size=(W_guess, H_guess, one_hots.shape[0]),  # (width, height, modalities=M)
            dx=self.dx,
            num_controls=0,
            max_levels=4,
            agents=None,
