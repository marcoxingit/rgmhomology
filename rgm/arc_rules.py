...
# ---------- Border / dynamic crop detection ----------
def detect_crop_or_border(
    inp: np.ndarray, out: np.ndarray, background: int
) -> Tuple[str, Tuple, int]:
    """
    Prefer dynamic foreground crop ('crop_fg') when exact.
    Else try border padding with non-negative margins (computed for this pair).
    Returns (kind, params, color) where:
      - 'crop_fg'  : params = ()
      - 'border'   : params = (top, left, bottom, right), color = int
      - 'none'     : params = (), color ignored
    """
    # Dynamic foreground crop check
    cropped, _bbox = crop_to_bbox(inp, background)
    if cropped.shape == out.shape and np.array_equal(cropped, out):
        return "crop_fg", (), 0

    # Border padding (pair-specific)
    H, W = inp.shape
    Ho, Wo = out.shape
    if Ho >= H and Wo >= W:
        color = infer_background_color(out)
...
class Rule:
    def __init__(
        ...
        self.colormap = dict(colormap) if colormap else {}
        self.translation = translation  # (dy, dx)
        self.post = post  # 'none' | 'crop_fg' | 'border' | 'tile'
        self.post_params = post_params  # () or (t,l,b,r) or (H,W)
        self.post_color = int(post_color)
    ...
    def apply(self, grid: np.ndarray) -> np.ndarray:
        ...
        if self.translation != (0, 0):
            g = translate(g, self.translation[0], self.translation[1], self.background)
        if self.post == "crop_fg":
            g, _ = crop_to_bbox(g, self.background)
        elif self.post == "border":
            t, l, b, r = self.post_params
            g = add_border(g, t, b, l, r, self.post_color)
        elif self.post == "tile":
...
def infer_rule(
    ...
    for cm in colormaps:
        for d in dihedrals:
            base = apply_dihedral(x0, d)
            if cm:
                base = apply_color_map(base, cm)

            rule = None
            # If shapes match, try to learn a translation on the first pair
            if base.shape == y0.shape:
                dy, dx = best_translation(base, y0, bg0)
                rule = Rule(bg0, d, cm, (dy, dx))
            else:
                rule = Rule(bg0, d, cm, (0, 0))

            # Decide post-op using the pair-specific check
            rtype, params, color = detect_crop_or_border(rule.apply(x0), y0, bg0)
            if rtype != "none":
                rule.post = rtype
                rule.post_params = params
