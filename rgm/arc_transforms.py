...
# ---- Translation (same size, clipped & robust) ----
def translate(grid: np.ndarray, dy: int, dx: int, fill: int) -> np.ndarray:
    """Translate by (dy, dx). If overlap is empty (|dy|>=H or |dx|>=W), return all-fill grid."""
    H, W = grid.shape
    out = np.full((H, W), fill, dtype=grid.dtype)
    if H == 0 or W == 0:
        return out
    ...
    # If shift fully moves content off the canvas, just return background
    ...
    out[y0:y1, x0:x1] = grid[sy0:sy1, sx0:sx1]
    return out
